    ...
    <div className="min-h-screen relative overflow-hidden bg-background text-foreground">
      ...
      <section className="pt-48 pb-32 relative z-10">
        <div className="container mx-auto px-4">
          <Link to="/#products" className="inline-flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.4em] text-muted-foreground/60 hover:text-primary transition-all mb-20 group italic">
            <ArrowLeft size={16} className="group-hover:-translate-x-2 transition-transform" strokeWidth={3} /> Return to Operations
          </Link>
          ...
          <div className="grid lg:grid-cols-2 gap-32 items-center">
            ...
            >
              ...
              <img
                ...
                className="rounded-[3rem] shadow-2xl relative z-10 border border-white/10 group-hover:border-primary/40 transition-all duration-1000"
                width={1280}
                height={720}
              />
            </motion.div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-12 max-w-7xl mx-auto">
            {features.map((f, i) => (
              <motion.div
                ...
                whileHover={{ y: -16 }}
                className="glass-card !bg-white/[0.03] p-12 group transition-all duration-700 border-white/10 hover:border-primary/40 shadow-2xl h-full"
              >
                <div className="w-16 h-16 rounded-2xl bg-white/5 flex items-center justify-center mb-8 group-hover:bg-primary transition-all duration-500 shadow-2xl group-hover:rotate-[10deg]">
      ...
      {/* Yield & Analysis */}
      <section className="py-40 relative z-10 bg-white/[0.02] border-y border-white/5">
        <div className="container mx-auto px-4">
          <div className="grid lg:grid-cols-2 gap-32 items-center">
            <motion.div 
              initial={{ opacity: 0, x: -60 }} 
              ...
              <div className="space-y-8 max-w-xl">
                {benefits.map((b, i) => (
                  <motion.div 
                    ...
                    className="flex gap-8 p-10 rounded-[2.5rem] bg-white/[0.03] border border-white/5 hover:border-primary/20 transition-all duration-500 group"
                  >
            ...
            >
              <div className="relative group">
                ...
                <img
                  ...
                  className="rounded-[3.5rem] shadow-2xl relative z-10 border border-white/10 group-hover:border-primary/40 transition-all duration-1000"
                />
              </div>
              
              {screenshots.length > 1 && (
                <div className="flex gap-6 p-6 rounded-[2.5rem] bg-white/5 max-w-fit mx-auto border border-white/5">
                  {screenshots.map((s, i) => (
                    <button
                      key={i}
                      onClick={() => setActiveScreenshot(i)}
                      className={`w-32 h-20 rounded-2xl overflow-hidden border-2 transition-all p-1 ${i === activeScreenshot ? "border-primary scale-110 shadow-2xl shadow-primary/20" : "border-transparent opacity-40 hover:opacity-100"}`}
                    >
      ...
      <section id="showcase" className="py-40 relative z-10">
        <div className="container mx-auto px-4">
          <div className="max-w-7xl mx-auto glass-card !bg-white/[0.03] p-1.5 rounded-[4rem] shadow-2xl border-white/5 overflow-hidden">
            <div className="grid lg:grid-cols-2 h-full">
              <div className="bg-foreground p-24 rounded-[3.5rem] text-background flex flex-col justify-between relative overflow-hidden group">
                ...
                <div className="relative z-10">
                  <h3 className="text-4xl sm:text-5xl font-heading font-black mb-12 leading-[0.9] italic tracking-tighter">Witness <br />Dominion.</h3>
                  <p className="text-background/60 mb-16 max-w-sm text-lg sm:text-xl font-medium tracking-tight leading-tight">Watch how {name} empowers global titans to secure their position through absolute architectural precision.</p>
                  
                  <div className="space-y-8">
                    <div className="flex items-center gap-6 group/item">
                      <div className="w-14 h-14 rounded-2xl bg-background/5 text-background flex items-center justify-center font-black text-xl italic group-hover/item:bg-primary group-hover/item:text-white transition-all duration-500">01</div>
                      <span className="text-[10px] font-black uppercase tracking-[0.4em] opacity-80">Market Synchronization</span>
                    </div>
                    <div className="flex items-center gap-6 group/item">
                      <div className="w-14 h-14 rounded-2xl bg-background/5 text-background flex items-center justify-center font-black text-xl italic group-hover/item:bg-primary group-hover/item:text-white transition-all duration-500">02</div>
                      <span className="text-[10px] font-black uppercase tracking-[0.4em] opacity-80">Yield Optimization</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-20 relative z-10">
                  <div className="flex -space-x-6 mb-8 group-hover:space-x-0 transition-all duration-700">
                    {[1, 2, 3, 4, 5].map((i) => (
                      <img key={i} src={`https://i.pravatar.cc/120?img=${i + 30}`} className="w-16 h-16 rounded-[1.5rem] border-4 border-background shadow-2xl" alt="Titan" />
                    ))}
                  </div>
              ...
              <div className="p-20 md:p-32 flex flex-col justify-center">
                <div className="space-y-12">
                  {[
                    ...
                    <div key={i} className="flex items-center gap-8 p-10 rounded-[2.5rem] bg-white/5 border border-white/5 hover:border-primary/20 transition-all duration-700 cursor-pointer group shadow-2xl">
                      ...
                      <div>
                        <h4 className="text-3xl font-heading font-black text-foreground group-hover:text-primary transition-colors italic tracking-tighter leading-none mb-3">{video.title}</h4>
                        <p className="text-[10px] font-black uppercase tracking-[0.4em] text-muted-foreground/60 italic">Blueprint Series • {video.duration}</p>
                      </div>
                    </div>
                  ))}
                  
                  <div className="pt-10">
                    <p className="text-[10px] font-black uppercase tracking-[0.5em] text-center opacity-20">All Showcases are Architect-Led</p>
                  </div>
                </div>
              </div>
            </div>
