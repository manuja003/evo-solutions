...
(function initParticles() {
  ...
  for (let i = 0; i < count; i++) {
    particles.push({ x: rnd(0, canvas.width), y: rnd(0, canvas.height), r: rnd(1, 2.5), vx: rnd(-0.2, 0.2), vy: rnd(-0.3, -0.05), alpha: rnd(0.1, 0.4), color: Math.random() > 0.5 ? '16,185,129' : '37,99,235' });
  }
  function draw() {
...
(function initChartBars() {
  ...
  const heights = [35, 55, 42, 70, 50, 85, 60, 78, 45, 90];
  bars.forEach((bar, i) => {
...
(function initForm() {
  ...
  const fields = {
    ...
    company:  { el: document.getElementById('company'),  err: document.getElementById('err-company'),  validate: v => v.trim().length >= 2 ? '' : 'Please enter your company or business name.' },
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (industry && industry.value) payload.append('Business Type', industry.value);
    if (demoDate && demoDate.value) payload.append('Preferred Demo Date', demoDate.value);
    if (message && message.value.trim()) payload.append('Additional Message', message.value.trim());
    try {
